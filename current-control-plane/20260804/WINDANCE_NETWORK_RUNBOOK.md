...
- Vega/Codex is Herald's VP of Technology and internal technical lead, working behind Herald's single user-facing front door.
...
- Herald is the single user-facing assistant. The Hermes dashboard is the runtime/admin shell; its normal `/sessions` front door now redirects to the Herald `/chat` page. Use `/hermes-admin` (or `/sessions?admin=1`) only for explicit Hermes maintenance/provider administration.
...
- After transcription, `/voice/ask` defaults to Herald mode. Herald owns the conversation and creates internal Vega/Forge staff work when the request needs implementation.
...
- Walkie behavior: every request enters Herald first. Herald answers immediately when possible and creates real internal Vega/Forge staff tasks for build/fix/install/restart/configure/Node-RED/Cloudflare/service work, then reports the result back through the same front door.
