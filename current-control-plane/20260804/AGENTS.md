...
Also check `vega-task-inbox/VEGA_TASK_INBOX.md` and, when Herald is reachable, `/Users/herald/knowledge/VEGA_TASK_INBOX.md` for Herald-created Vega/Forge staff tasks that may need Codex/Vega attention.
...
- Herald runs LaunchAgent `com.windance.vega-task-bridge` every 120 seconds. It executes `/Users/herald/services/vega-task-bridge/vega_task_bridge.py`, polls Herald's local `staff_tasks` queue for `Vega` and `Forge`, updates `/Users/herald/knowledge/VEGA_TASK_INBOX.md`, and sends William a Max/iMessage cage-rattle notice for newly detected Vega/Forge task events. Vega should oversee Forge task outcomes for now.
- The older HAL Windows Scheduled Task `WindanceVegaTaskBridge` is disabled because periodic PowerShell launches could steal focus. Do not re-enable it unless William explicitly asks to move the bridge back to HAL.
