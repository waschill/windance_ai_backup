...
def claude_review(ops, payload):
    ...
    wrapper='/Users/herald/services/claude-review/claude_review.py'
    output=execute_bounded(['/Users/herald/.hermes/hermes-agent/venv/bin/python',wrapper,'--query-file','-'],
                           review_prompt(proposal,'Claude'),270)
    # Quiet wrapper emits final prose only; accept strict JSON, optionally one fenced block.
    clean=output.strip()
    if clean.startswith('```json\n') and clean.endswith('\n```'): clean=clean[8:-4]
    verdict=validate_verdict(json.loads(clean),proposal,'Claude')
...
def authorized_recovery(ops, sha):
    ...
    if ops.run(['/bin/launchctl','print',cmd[-1]]).returncode: raise ValueError('Service registration changed')
