...
- William's numbered Gmail commands accept both the long forms and case-insensitive shortcuts: `AlDel` = Always Delete, `NoDel` = Notify Delete, and `Red` = Mark Read. These aliases enter the deterministic Gmail-control lane and preserve the same sender-rule and approval boundaries as their long forms.
