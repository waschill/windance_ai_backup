...
The authenticated Operations page at `https://herald.reflectsody.com/operations` now uses a compact, text-based network telemetry strip across the top, a dense expandable Open Work list on the right, and a conventional chat layout. The staff selector is the first control in the chat tile; the heading follows immediately, the transcript occupies the flexible middle, and the composer remains at the bottom. New user, staff, and working-indicator entries automatically scroll into view. Page colors now inherit Hermes theme variables with fallbacks instead of relying entirely on a fixed palette. Independent data fetching prevents one unavailable service from blanking unrelated telemetry or staff data.
